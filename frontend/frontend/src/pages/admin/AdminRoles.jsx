  ...
  return (
    <AdminLayout active="roles">
      <header className="rp-heading">
        <div className="rp-heading-row">
          <h1>Gestión de Roles y Permisos</h1>
          <span className="rp-readonly-badge">Modo Supervisión</span>
        </div>
                  ...
                  <tr key={usuario.id}>
                    ...
                    <td>
                      <div className="rp-actions">
                        <button type="button" disabled title="Vista de supervisión" onClick={() => abrirEditar(usuario)} aria-label={`Editar rol de ${usuario.nombre}`}>
                          &#9998;
                        </button>
                        {usuario.estado === 'Activo' && (
                          <button className="rp-deactivate" type="button" disabled title="Vista de supervisión" onClick={() => setUsuarioDesactivar(usuario)} aria-label={`Desactivar ${usuario.nombre}`}>
                            &#9817;&times;
                          </button>
                        )}
                      </div>
