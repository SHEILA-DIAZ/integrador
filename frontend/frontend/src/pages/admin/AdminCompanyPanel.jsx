...
const formatearSoles = (valor) =>
  `S/ ${numero(valor).toLocaleString('es-PE', {
    maximumFractionDigits: 0,
  })}`

const nombreCampana = (campana) =>
  campana.titulo || campana.nombre || campana.campaignName || campana.campaign_name || 'Campaña sin nombre'

const nombreCorto = (texto) => {
  const valor = String(texto || '')
  return valor.length > 24 ? `${valor.slice(0, 24)}...` : valor
}
...
const normalizarDonacion = (donacion) => ({
  ...
  fecha: donacion.fecha || donacion.date || donacion.created_at || donacion.createdAt,
})
...
function Icono({ tipo }) {
  ...
  return (
    <svg className="acp-icon" aria-hidden="true" viewBox="0 0 24 24" fill="none">
      {paths[tipo]}
    </svg>
  )
}
...
function LineChart({ values, metaValues, area = false, compact = false }) {
  ...
  const point = (value, index) => {
    const x = left + (index * chartWidth) / (values.length - 1 || 1)
  ...
  return (
    <svg className="acp-line-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Grafico de donaciones">
      {[0, 1, 2, 3, 4].map((linea) => {
        ...
        return (
          <g key={linea}>
            <line className="grid" x1={left} x2={width - right} y1={y} y2={y} />
            <text x={left - 8} y={y + 4} textAnchor="end">{formatearEntero(valor / 1000)}K</text>
          </g>
        )
      })}
      {meses.map((mes, index) => {
        const x = left + (index * chartWidth) / (meses.length - 1)
        return <text key={mes} x={x} y={height - 10} textAnchor="middle">{mes}</text>
      })}
    ...
    <div className="acp-bar-chart" role="img" aria-label="Progreso de campañas">
      ...
      <div className="acp-bars">
        {visibles.map((campana) => {
          ...
          return (
            <div className="acp-bar-item" key={nombreCampana(campana)}>
              <span style={{ height: `${Math.max(6, progreso)}%` }} />
              <small>{nombreCorto(nombreCampana(campana))}</small>
            </div>
          )
        })}
...
export default function AdminCompanyPanel() {
  ...
  useEffect(() => {
    ...
    const cargarDatos = async () => {
      ...
      try {
        const [reporteResponse, donacionesResponse] = await Promise.allSettled([
        ...
        if (donacionesResponse.status === 'fulfilled') {
          setDonaciones(normalizarLista(donacionesResponse.value.data).map(normalizarDonacion))
        } else {
          try {
            const donacionesPublicas = await obtenerDonacionesVirtualesPublicas()
            if (!cancelado) setDonaciones(normalizarLista(donacionesPublicas.data).map(normalizarDonacion))
          } catch {
  ...
  const donacionesMensuales = useMemo(() => {
    if (totalParaGraficos <= 0) return [0, 0, 0, 0, 0, 0]
    return factoresMensuales.map((factor) => Math.round(totalParaGraficos * factor))
  }, [totalParaGraficos])

  const metaMensual = useMemo(() => {
    if (metaCombinada <= 0) return null
    return factoresMensuales.map((factor) => Math.round(metaCombinada * factor))
  }, [metaCombinada])

  const actividad = useMemo(() => {
    const desdeDonaciones = donaciones.slice(0, 4).map((donacion, index) => ({
      nombre: donacion.donante,
      detalle: `${nombreCorto(donacion.campana)} • ${index === 0 ? 'Hace 10 min' : `Hace ${index + 1} horas`}`,
      monto: donacion.monto,
    }))

    if (desdeDonaciones.length > 0) return desdeDonaciones

    return campanasDestacadas.slice(0, 4).map((campana, index) => ({
      nombre: nombreCampana(campana),
      detalle: `Campaña destacada • Hace ${index + 1} horas`,
      monto: obtenerRecaudado(campana),
    }))
  }, [campanasDestacadas, donaciones])

  const mostrarToastCreacion = () => {
    setToast('Funcionalidad de creación de campaña disponible para admins de compañía.')
    window.setTimeout(() => setToast(''), 3600)
  }

  const acciones = [
    { label: 'Crear Campaña', icon: 'plus', primary: true, onClick: mostrarToastCreacion },
    { label: 'Gestionar Campañas', icon: 'heart', onClick: () => navigate('/admin/campaigns') },
    { label: 'Historial Donaciones', icon: 'clock', onClick: () => navigate('/admin/donations') },
    { label: 'Usuarios Internos', icon: 'users', onClick: () => navigate('/admin/users') },
    { label: 'Análisis Ingresos', icon: 'money', onClick: () => navigate('/admin/cash-income') },
  ]

  return (
    <AdminLayout active="solicitudes">
      <section className="acp-page">
        {toast && (
          <div className="acp-toast" role="status">
            <Icono tipo="info" />
            <span>{toast}</span>
          </div>
        )}
        ...
        <header className="acp-hero">
          ...
          <div>
            ...
            <p>Visualización única para Super Admin</p>
          </div>
        </header>

        {mensaje && <div className="acp-alert">{mensaje}</div>}
        {loading && <div className="acp-state">Cargando Panel Compañía...</div>}

        <section className="acp-kpis" aria-label="Resumen principal">
          <KpiCard label="Total Recaudado" value={formatearSoles(totalRecaudado)} note="Dato real del reporte global" color="red" icon="money" />
          <KpiCard label="Campañas Activas" value={formatearEntero(campanasActivas)} note="Dato real del reporte global" color="blue" icon="heart" />
          <KpiCard label="Total Donaciones" value={formatearEntero(totalDonaciones)} note="Dato real del reporte global" color="green" icon="target" />
          <KpiCard
            label="Meta Alcanzada"
            value={metaAlcanzada === null ? '--' : `${metaAlcanzada}%`}
            note={metaCombinada > 0 ? `de ${formatearSoles(metaCombinada)}` : 'Sin meta disponible'}
            color="purple"
            icon="bolt"
          />
        </section>
        ...
        <section className="acp-grid acp-grid-two">
          <article className="acp-card">
            ...
            <p>Evolución de recaudación mensual</p>
            <LineChart values={donacionesMensuales} />
          </article>

          <article className="acp-card">
            <h2>Progreso de Campañas</h2>
            <p>Porcentaje alcanzado por campaña</p>
            <BarChart campanas={campanasDestacadas} />
          </article>
        </section>

        <section className="acp-grid acp-grid-summary">
          <article className="acp-card">
            <div className="acp-card-row">
              <div>
                <h2>Progreso Mensual</h2>
                <p>Recaudación real vs meta mensual</p>
              </div>
              {metaAlcanzada !== null && <span className="acp-pill"><Icono tipo="trend" /> Meta {metaAlcanzada >= 100 ? 'superada' : 'en avance'}</span>}
            </div>
          ...
          <article className="acp-card acp-summary-card">
            <div className="acp-summary-head">
              <span><Icono tipo="award" /></span>
              <div>
        ...
        <section className="acp-grid acp-grid-two">
          <article className="acp-card">
            ...
            <p>Últimas donaciones recibidas</p>
            <div className="acp-activity">
              {actividad.length === 0 ? (
                <div className="acp-empty">Sin actividad reciente disponible.</div>
              ) : actividad.map((item) => (
                <div className="acp-activity-row" key={`${item.nombre}-${item.detalle}`}>
                  <div>
                    <strong>{item.nombre}</strong>
                    <span>{item.detalle}</span>
                  </div>
                  <b>{formatearSoles(item.monto)}</b>
                </div>
              ))}
            </div>
          </article>
          ...
          <article className="acp-card">
            ...
            <p>Tus campañas más importantes</p>
            <div className="acp-campaigns">
              {campanasDestacadas.length === 0 ? (
                <div className="acp-empty">Sin campañas destacadas en el reporte.</div>
              ) : campanasDestacadas.slice(0, 3).map((campana) => {
                const progreso = progresoCampana(campana)
                return (
                  <div className="acp-campaign" key={nombreCampana(campana)}>
                    <div>
                      <strong>{nombreCampana(campana)}</strong>
                      <span>{progreso === null ? '--' : `${progreso}%`}</span>
                    </div>
                    <i><span style={{ width: `${progreso ?? 0}%` }} /></i>
                    <small>{formatearSoles(obtenerRecaudado(campana))}<b>Meta: {obtenerMeta(campana) > 0 ? formatearSoles(obtenerMeta(campana)) : '--'}</b></small>
                  </div>
                )
              })}
            </div>
        ...
        <section className="acp-actions" aria-label="Acciones del panel compañía">
          {acciones.map((accion) => (
            <button className={accion.primary ? 'primary' : ''} type="button" key={accion.label} onClick={accion.onClick}>
              <Icono tipo={accion.icon} />
