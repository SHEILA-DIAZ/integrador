...
export default function Registro() {
  ...
  const validar = () => {
    ...
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email)) nuevosErrores.email = 'Ingresa un email válido'
    if (!form.telefono.trim()) nuevosErrores.telefono = 'El teléfono es requerido'
    else if (!/^\d{9}$/.test(form.telefono)) nuevosErrores.telefono = 'El teléfono debe tener 9 dígitos'
    ...
    if (!form.confirmarPassword) nuevosErrores.confirmarPassword = 'Confirma tu contraseña'
    else if (form.confirmarPassword !== form.password) nuevosErrores.confirmarPassword = 'Las contraseñas no coinciden'
    ...
    <main className="rd-page">
      ...
      <section className="rd-hero">
        <div className="rd-hero-overlay">
          <div className="rd-heart" aria-hidden="true">
        ...
        <form className="rd-card" onSubmit={handleSubmit} noValidate>
          {exito ? (
            <div className="rd-success-card">
              ...
              <button className="rd-submit" type="button" onClick={() => navigate('/login')}>
                Ir a Iniciar Sesión
              </button>
            </div>
          ) : (
