...
const normalizarDonacion = (donacion) => ({
  ...
  comprobanteUrl: obtenerUrlComprobante(donacion),
})
...
export default function DonationHistory() {
  ...
  useEffect(() => {
    ...
    const cargarHistorial = async () => {
      setLoading(true)
      try {
        const [historial, campanasActivas] = await Promise.allSettled([
          obtenerHistorialDonaciones(),
          obtenerCampanas({ estado: 'ACTIVA' }),
        ])

        if (cancelado) return

        if (historial.status === 'rejected') throw historial.reason

        setDonaciones(
          normalizarLista(historial.value.data, ['donations', 'donaciones', 'history', 'historial'])
            .map(normalizarDonacion)
        )

        if (campanasActivas.status === 'fulfilled') {
          setCampanas(
            normalizarLista(campanasActivas.value.data, ['campaigns', 'campanas'])
              .map(obtenerNombreCampana)
  ...
  const descargarComprobante = async (donacion) => {
    if (!donacion.id) {
      setMensaje('No se encontró el identificador de la donación.')
      return
    }
    ...
    try {
      let comprobanteUrl = donacion.comprobanteUrl

      if (!comprobanteUrl) {
        const response = await obtenerComprobanteDonacion(donacion.id)
        comprobanteUrl = obtenerUrlComprobante(response.data?.data || response.data)
      }

      if (!comprobanteUrl) throw new Error('El comprobante PDF todavía no está disponible.')
      window.open(comprobanteUrl, '_blank', 'noopener,noreferrer')
    } catch (err) {
      setMensaje(obtenerMensajeApi(err, err.message || 'No pudimos descargar el comprobante.'))
    } finally {
  ...
  return (
    <main className="dh-page">
      <header className="dh-nav">
        ...
        <nav className="dh-links" aria-label="Navegación principal">
