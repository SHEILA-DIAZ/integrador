  ...
  useEffect(() => {
    ...
    const cargarSolicitudes = async () => {
      ...
      try {
        ...
        if (!cancelado) {
          setSolicitudes([])
          setMensaje(obtenerMensajeApi(err, 'No se pudieron cargar las solicitudes. Intenta nuevamente.'))
        }
      } finally {
              ...
              <tbody>{solicitudes.map((solicitud) => {
                ...
                return (
                  <tr key={solicitudId || `${getNombre(solicitud)}-${solicitud.ruc}`}>
                    ...
                    <td>{pendiente ? <div className="ps-actions">
                      <button className="ps-action ps-action-approve" type="button" disabled={procesandoId === solicitudId} onClick={() => setSolicitudAprobar(solicitud)}>✓ Aprobar</button>
                      <button className="ps-action ps-action-reject" type="button" disabled={procesandoId === solicitudId} onClick={() => abrirRechazo(solicitud)}>⊗ Rechazar</button>
                    </div> : <span className="ps-no-action">-</span>}</td>
                  </tr>
                )
              })}</tbody>
            </table>
